...
- 两个入口复用现有完成预检和完成接口，项目始终由HR手动结束。确认窗以最新预检判断是否提前结束，明确显示将关闭的任务数量、报告使用的已提交答卷数量及不可重开后果，并继续要求填写原因和勾选二次确认。页面统计与预检之间发生变化时，以最新预检为准，不直接发送完成请求。
- 数据加载、预检、完成请求进行中及完成结果未知时锁定完成入口；权限不足、数据范围不完整或项目已完成时不提供完成或提前结束操作。保留原有后端锁版本、预期统计校验和完成后关闭未提交任务的规则。
