...
上述13张表已经由Alembic revision `20260902_02_feedback_domain`实现。完整字段类型、可空性、默认值、外键、唯一约束和中文注释见[P2领域表与字段字典](./10-p2-field-dictionary.md)。这只表示领域持久化骨架已经建立，不表示后续业务API和页面已经完成。
...
P5-00已新增revision `20260902_04_feedback_publication`，`down_revision`为`20260902_03_feedback_designer`。独立开发库和测试库均已升级到该唯一head，当前数据库为14张评价领域表。

已实施变更：

- 新增`fb_evaluator_selection`评价人选择配置表，保存准备期可恢复、发布后随问卷版本冻结保留的“项目、版本、被评价人、关系、评价人”配置。
- 该表不是员工正式任务；`fb_assignment`继续只在发布事务内生成。
- 选择表使用目标和关系复合外键、评价人用户外键、与正式任务一致的业务唯一键、RuoYi审计字段及聚合查询索引。
- 为现存`DRAFT`版本确定性补齐上级、同级、下级、自己、其他五类固定关系；除自己外初始禁用且权重为0，不引入组织级默认权重。
- 新项目创建事务同步写入固定关系；自己关系始终启用、权重0。按目标派生自评选择仍属于P5-03，P5-00没有提前创建目标或正式任务。
- 当前评价领域表为14张，模型和数据库中文注释字段为196个；具体字段和约束见[P2字段字典的P5增量契约](./10-p2-field-dictionary.md)与[P5技术预检](./12-p5-publication-precheck.md)。

独立测试库已完成P4→P5→P4→P5有数据往返：P4既有项目、页面和目标数据保留，选择表按降级语义移除并可重建，五类固定关系再次升级后没有重复。后续P5-01至P5-08现已实现发布Service，并通过单事务冻结、任务生成、幂等并发和故障回滚门禁，完整证据见[实施路线图P5验收记录](./09-implementation-roadmap.md#105-p5-01至p5-08执行与验收记录)。

### 8.5 P6已实施迁移

迁移链新增`20260902_05_feedback_answering`和`20260902_06_feedback_permissions`。开发库/测试库已升级到后者唯一head，领域表数量和中文注释数量保持14张/196个。
