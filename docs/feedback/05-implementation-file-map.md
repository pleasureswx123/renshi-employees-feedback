...
本清单同时登记已落地文件和下一阶段计划。标记“计划”的路径尚未创建，不代表已经实现；P7回收进度与手动完成文件已落地，功能完成状态与直接验证证据以实施路线图为准。
...
│  ├─ feedback_p6_e2e_fixture.py             # 已存在：P6/P7隔离库浏览器夹具、数据范围、审计对账和精确清理
...
└─ tests/module_feedback/
   ├─ controller/                            # 已存在：模块入口、项目、草稿及P7进度/完成API测试
   ├─ enums/                                 # 已存在：枚举契约测试
   ├─ service/                               # 已存在：状态门禁及P3至P7真实PostgreSQL服务/并发测试
   ├─ entity/                                # 已存在：模型、约束元数据、P3/P4 VO及P5固定关系测试
   ├─ dao/                                   # 已存在：DAO单元及真实PostgreSQL事务测试
   └─ migration/                             # 已存在：结构、P6权限、P7审计迁移和模型扫描门禁测试
```
...
│  ├─ router/
│  │  └─ index.js                           # 已存在：固定路由、权限守卫和P5发布/P7进度路由
│  ├─ stores/
...
└─ tests/
   ├─ api/                                   # 已存在：登录、项目、草稿、P5发布及P7进度/完成测试
   ├─ components/                            # 已存在：工作台、五题型、P5发布及P7进度组件测试
   ├─ constants/feedbackEnums.test.js        # 已存在：P2枚举契约测试
   ├─ stores/                                # 已存在：权限、P4草稿、P5发布、P6答题和P7进度Store测试
   ├─ utils/                                 # 已存在：四位定点数和P5配置协议测试
   ├─ e2e/                                   # 已存在：登录、P4、P5发布及P7进度/完成浏览器测试（Mock接口）
   └─ live-e2e/employee-answering.spec.js    # 已存在：P6/P7真实发布、答题、数据范围、完成和数据库对账
```
