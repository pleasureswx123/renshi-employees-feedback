...
指标创建、权重及计分题绑定集中在第二步，题目属性不再提供指标选择或提示用户先添加指标；第二步列出未完成项。权重精确合计100%、所有计分题绑定且每个指标有有效计分题后，“下一步：配置人员”先保存草稿，并以后端返回的`isPublishReady`及问题清单再次检查，通过后才进入人员关系配置；失败留页并保留草稿。仍可返回问卷修改，新增或删除题目后重新计算待办。该顺序是编辑器的交互引导，不新增业务状态或替代P5发布校验；项目列表中的人员配置入口继续遵循已有权限，正式发布仍由后端统一检查与冻结。
...
2026-09-07编辑空间调整：标题与说明置于Element Plus折叠面板，初始展开，用户可主动收起且不卸载表单；标题缺失时保存或下一步自动展开并显示校验。普通属性栏收窄，实时预览仍按既有规则加宽。右侧显示全卷连续题号，大纲操作在悬停或键盘聚焦时突出。当前步骤显示“当前”，离开问卷步骤后才显示“已检查”；发布门禁与业务状态不变。保存状态保留固定空间；原始满分仅在显示时去除小数末尾零，不改变计算或持久化值。
...
2026-09-10流程轴约定：问卷编辑与人员发布共用六步流程：编辑问卷、配置指标、评价谁、设置评价关系、谁来评价、检查并发布。问卷页负责第1至2步，点击后续步骤先校验并保存；人员页沿用第3至6步编号，返回指标时直接打开对应面板。各步复用原有校验、权限与未保存保护；仅自评自动跳过第5步。

2026-09-03交互增量约定：左侧采用与右侧一致的竖排标签及紧凑展开/收起控制条，标签靠最左侧，题型面板在上、问卷大纲在下；默认展开题型面板，切换及折叠保留当前页、题目与编辑状态。左侧大纲用页图标、题型图标、题量和选中状态导航，页面新增/排序/删除采用带提示的图标按钮；中间画布直接修改；右侧默认折叠，仅显示靠右竖排的“题目属性、评价指标、实时预览”标签。点击标签展开对应面板，标签始终位于内容最右侧；顶部始终保留22px高的紧凑控制条，折叠时显示向左展开图标、展开时显示向右收起图标，按钮内边距5px；标签纵向位置不随展开收起变化，再次点击当前标签也可收起；切换标签保留编辑与试填状态。折叠时释放宽度给编辑画布，点击“下一步：配置指标”主动展开指标面板。选择实时预览时按可用宽度加宽右栏，切回属性或指标时恢复原宽度；窄PC窗口仍保留画布的编辑空间。首期仅通过PC登录作答，移除独立电脑/手机预览入口和弹窗。实时预览使用 `QuestionnairePreviewContent`、现有 `QuestionRenderer` 和五种题型组件。页码、分页控件与重新试填操作位于通栏分隔线上方，页码切换同步大纲与画布；正文不显示页名，每页均展示问卷标题和说明，切页时预览回到问卷开头。试填状态仅存于预览组件内，切页、页签切换及文案修改保留试填，答题约束变更清除该题试填并重建控件，其他题不受影响。预览不再要求题目内容完整；空内容使用显示占位，绝不写入业务字段。

单选题的新建默认选项、新增选项、批量设置省略分值均为2分，输入最低1分。批量设置逐行校验正整数，显式0分或小数均拒绝；后端服务保存前和发布检查复用规则。旧问卷读取仍保留原分值，编辑时提示不符合新规则的分值；不改写冻结问卷、原始答案或历史计分。数据库类型与历史数据不迁移。
...
- 页面支持新增、上移、下移和删除，不提供重命名；草稿页名按当前顺序固定为“第1页、第2页……”并在新增、移动、删除、加载及保存时保持连续。已有自定义草稿页名在编辑器中按顺序归一化，下次保存时写入；不修改已发布的冻结问卷。页面不设说明，整份问卷共用问卷说明。
...
- 发布完整性检查：指标权重总和精确等于`100.0000`；所有`isScored=true`题目恰好绑定一个指标；每个指标至少包含一道最高分大于0的计分题，不能只绑定问答题。
...
- `QuestionnaireOutline.vue`：页面、题目选择和移动入口；页面标题按所在位置自动编号，不提供自定义标题入口。
...
### 2026-09-11 指标表单与目录调整

指标名称、权重、绑定计分题为必填项，每个指标至少绑定一道有效计分题（含零权重指标），全部指标权重合计精确为100%后方可进入下一步或发布。草稿仍可暂存未完成的绑定配置。目录统一提供新增入口，并展示序号、名称、绑定题数、权重及选中状态；主区域不再重复提供新增按钮。
