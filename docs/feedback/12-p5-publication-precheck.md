...
2026-09-10 准备流程交互约定：六步流程点击“下一步”或步骤轴向前跳转时，先检查前置步骤条件，再通过现有草稿接口保存配置，保存成功后才进入目标步骤。校验失败提示具体问题；保存失败保留当前步骤和草稿；保存期间禁止重复提交。问卷进入指标步骤只要求问卷满足本步条件，允许保存尚未完成指标绑定的草稿。最终发布仍执行后端完整性检查与二次确认。返回上一步保留当前编辑内容，页面初次恢复指标步骤不触发保存。

评价方式选择约定：未启用任何他评关系时，不能据此自动选中“仅自评”；展示未选择状态，并要求用户明确选择“自评 + 他人评价”或“仅自评”后继续。已有启用的他评关系仍恢复为他评模式。现有数据未单独记录用户选择意图，因此重新进入纯自评草稿时也需要重新确认评价方式；已发布项目保持冻结只读。

2026-09-11 评价人选择：复用组织树穿梭组件，左侧按组织查找和勾选人员，右侧展示当前被评价人、当前关系的已选名单，支持批量添加与移除。同级额外提供“添加整部门”，包含可见下级部门，逐页获取全部候选人员后统一加入；自动排除被评价人本人、失效人员及重复人员。部门批量选择是当前人员名单快照，不建立动态部门绑定，也不自动推断评价关系。切换被评价人或关系后丢弃未完成的旧选择请求；加载失败不添加部分名单。保存和发布继续复用原评价人配置接口。
...
- 每位被评价人必须为每个已启用的非自评关系配置至少一名评价人，包括不计分关系；不允许以其他被评价人的分配抵消缺口。
- 每个目标始终至少有自动自评任务。
- 仅在未启用任何非自评关系的仅自评项目中，允许被评价人只有自评任务；他评项目中完全未分配或部分漏配均阻止发布。
...
- 他评关系权重使用四位定点数预检；每位被评价人都必须配齐每个启用关系的评价人；任何缺失组合均阻止发布，并定位具体人员和关系。仅自评项目不要求他评人员。
...
### 2026-09-11 逐人关系完整性校验

前端下一步与后端保存预检、正式发布统一检查“被评价人 × 已启用非自评关系”。缺失项返回 `TARGET_RELATION_ASSIGNMENT_REQUIRED`，路径为 `targets.{i}.relations.{relationCode}`。草稿允许保存但不得标记发布就绪。已有冻结项目和历史计分结果不追溯修改。

### 2026-09-11 六步统一配置工作区

- 统一入口为 `/hr/projects/:projectId/editor?step=1..6`，由 `ProjectConfigurationView.vue` 提供固定项目标题与工作区；原 publication 链接兼容重定向，已有入口无需失效。
- 第一步仅显示题型、大纲、问卷画布、题目属性和预览，不展示评价指标页签。
- 第二步以指标目录、指标名称与权重、计分题绑定为主内容，右侧复用问卷编辑器的真实题型预览，保留题号、选项和必答提示；绑定题目后同步预览，支持独立试填但不修改问卷配置。题目设置仍只在第一步提供。复用原 IndicatorPanel 与草稿接口。
- 第三至六步在同一工作区切换人员与发布组件，不再跳到另一个配置页面。前进经过原校验与保存，跨组件返回时先保存已有人员修改；保存失败保留当前步骤。
- URL 同步步骤；修改 URL 和浏览器导航经过现有步骤校验。指标页刷新会重新加载草稿并检查问卷条件。权限仍逐阶段控制，后端冻结及正式发布规则保持。
- 验证边界：通过组件测试验证布局分离、统一外壳、跨步骤保存与返回恢复；真实浏览器验收需单独执行。

第二步右侧与第一步实时预览保持一致：折叠栏、实时预览页签、分页工具栏、问卷标题说明及当前页全部题目；不再使用单题筛选预览。绑定题目时切换到对应页并高亮题目，预览试填与问卷配置隔离。

### 2026-09-11 他评初始占比调整

- 评价方式仍由用户手动选择。选择“自评＋他人评价”且尚无他评配置时，自动启用上级和同级，均计入总分，默认占比分别为60%和40%。下级和其他关系不默认启用。
- 已有手动配置及已保存的占比保持原值；默认值通过现有保存配置流程持久化。切换到仅自评清空他评后，再选择他评时重新使用上述默认值。后端创建草稿时仍保持未选择评价方式的初始关系状态。

### 2026-09-11 评价人抽屉配置

- 第五步主页面展示评价安排总览，点击“配置”、关系名称或“去配置”打开右侧抽屉，并定位被评价人及关系。只读状态通过“查看”打开相同抽屉。
- 抽屉保留组织树穿梭选择、同级整部门添加和关系切换；增删即时同步当前配置及总览。“完成”只关闭抽屉，结果仍通过页面保存配置持久化，离开第五步自动关闭抽屉。

### 2026-09-11 发布前完整预览

- 第六步在原有权威检查与任务汇总后展示完整问卷试填、指标占比与绑定题目、全部评价关系及逐人评价名单；使用 border-card 页签分别展示，默认打开问卷预览，各页签提供权限内的返回修改入口。
- 发布配置响应新增 questionnaire，只读复用同一次配置读取中的问卷及指标模型，与发布校验的数据一致；保存仍使用既有配置协议，不从预览写入问卷或答案。
- 返回修改后仍需保存和重新检查，正式发布沿用二次确认及冻结规则，不新增项目状态或发布后的修改阶段。

### 配置完成与正式发布分离

- 第六步为“预览并完成”，“完成配置”保存并检查，通过后返回项目列表，仍为准备阶段，不生成任务。未通过或保存失败保留本页供修改。
- 项目列表的准备阶段项目单独提供“发布项目”，仅发布权限可见。每次重新读取权威检查及版本锁，检查不通过可进入配置完善；通过后二次确认并调用原发布接口冻结配置、生成任务。取消或失败保留列表，成功刷新状态。
- 不新增项目状态，不混用“完成配置”与评价结束后的“完成项目”。

- 准备阶段列表提供“预览项目”，按发布配置读取权限打开只读抽屉，展示已保存的四类配置及校验缺项；不调用保存或发布接口。失败可重试，切换项目不沿用旧预览。已发布项目继续使用冻结配置查看入口。

- 同一被评价人的上级与同级评价人互斥：组织树、搜索和整部门添加均排除另一关系已选人员；存量冲突保留供用户选择移除哪一项，前端明确提示并阻止继续，后端发布校验同样拒绝冲突。不同被评价人之间不限制同一评价人担任不同关系。

- 评价人抽屉按已配置的关系顺序分步引导，展示人数与完成状态；下一步检查前置关系，完成此人检查全部启用关系。打开及切换被评价人时优先定位待完善项，可完成后直接配置列表下一位，保留已有选择。关闭抽屉仍可保留未完成草稿，正式保存由页面负责。
