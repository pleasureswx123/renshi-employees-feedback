...
- “评价谁”采用Element Plus组织树与已选人员列表的穿梭布局：展开部门按50人分页加载，可勾选多名员工批量添加或移除；搜索按账号/姓名查询，切换搜索不清空右侧名单，部门节点只导航、不隐式全选未加载人员。已选人员可搜索、单个或批量移除，继续复用原有保存、去重和移除目标时清理评价安排的规则。其他评价人选择沿用原表格。
- 组织树使用项目内`participant-departments`接口，只汇总RuoYi用户数据范围内的有效员工，返回所在部门及必要祖先；人员查询继续使用`participant-options`的`deptId`分页过滤，`unassigned=true`用于未分配部门的人员。不新增组织或人员表，不一次加载全组织员工，无人员管理权限不请求候选组织与人员。
- 默认展开公司和上级组织节点，直接展示部门列表；末级部门保持收起，展开后才加载其员工。2026-09-10本地验证：28项组件测试、10项后端测试（含真实隔离PostgreSQL数据范围查询）、4条隔离浏览器流程通过；本地真实页面只读确认默认部门层级及布局，无加载报错或横向溢出。本轮未提交、推送或部署。
