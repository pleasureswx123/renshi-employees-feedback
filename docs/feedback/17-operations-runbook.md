...
内置超级管理员（用户 ID 为 `1`）仅在该账号本人登录时出现在管理端用户查询中。其他账号即使拥有全部数据权限，也不能通过用户列表、搜索、导出、角色已分配/未分配人员列表或用户详情接口查看该账号。过滤在后端数据范围中执行，因此分页总数也排除该账号。

评价平台候选人员原有的超级管理员排除规则继续保留。该规则不删除账号、不改变登录认证，也不改写历史业务快照和审计日志；历史记录中的操作人不属于用户列表可见性控制范围。

用户查询缓存按登录用户隔离，默认有效期为 10 秒。更新后端后，已有缓存最多需等待该有效期结束再刷新页面。

验证：`tests/common/test_user_visibility.py` 覆盖普通账号、超级管理员、不同数据范围、部门查询不受影响和详情接口拒绝访问；SQL 筛选测试使用隔离的 SQLite 内存库，不代表生产 PostgreSQL 或真实浏览器验收。
