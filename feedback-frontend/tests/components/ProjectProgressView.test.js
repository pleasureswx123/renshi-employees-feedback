...
describe('HR项目回收进度页', () => {
  ...
  it('筛选和KPI点击请求服务端，完成按钮先实时预检再打开抽屉', async () => {
  ...
  it('无完成权限或已完成项目只读且不显示完成入口', async () => {
    ...
    expect(button('完成项目')).toBeUndefined()
    expect(button('返回回收进度入口')).toBeDefined()
  ...
  it('完成结果未知时锁定完成入口并只通过重新核对恢复实时预检', async () => {
    ...
    await flushPromises()
    ;[...drawer.querySelectorAll('button')].find(item => item.textContent.includes('确认完成项目')).click()
    await flushPromises()
    ...
    expect(button('完成项目').attributes('disabled')).toBeUndefined()
    expect(api.getProjectProgress).toHaveBeenCalledTimes(2)
    expect(api.completeProject).toHaveBeenCalledTimes(1)
  })
