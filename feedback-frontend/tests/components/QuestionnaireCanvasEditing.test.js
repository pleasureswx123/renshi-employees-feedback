...
describe('问卷画布直接编辑', () => {
  ...
  it('从人员页返回第2步时打开指标，问卷缺题时仍先定位问题', async () => {
    wrapper.unmount()
    ...
    await vi.waitFor(() => expect(activeCard().get('.el-form-item__error').text()).toBe('请填写题目内容'))
    expect(api.saveQuestionnaireDraft).not.toHaveBeenCalled()
  })

  it('六步轴返回问卷保留草稿，点击发布前先检查问卷和指标再保存', async () => {
  ...
  it('收起问卷信息保留内容，缺失标题时保存自动展开并阻止请求', async () => {
    ...
    await vi.waitFor(() => expect(wrapper.get('.questionnaire-form .el-form-item__error').text()).toBe('请填写问卷标题'))
    expect(api.saveQuestionnaireDraft).not.toHaveBeenCalled()
  })
  ...
  it('左栏默认展开题型面板，切换大纲与折叠不丢失当前题目', async () => {
    expect(wrapper.get('button[aria-label="收起左侧面板"]').attributes('aria-expanded')).toBe('true')
    expect(wrapper.findAll('.left-panel [role="tab"]').map(item => item.text())).toEqual(['题型面板', '问卷大纲'])
    expect(wrapper.get('#pane-types').isVisible()).toBe(true)
    await wrapper.get('#tab-outline').trigger('click')
    await flushPromises()
    expect(wrapper.get('#pane-outline').isVisible()).toBe(true)
    expect(wrapper.get('#pane-types').isVisible()).toBe(false)
    await wrapper.get('button[aria-label="收起左侧面板"]').trigger('click')
    expect(wrapper.get('.editor-grid').classes()).toContain('left-collapsed')
    await wrapper.get('button[aria-label="展开左侧面板"]').trigger('click')
    expect(wrapper.get('.editor-grid').classes()).not.toContain('left-collapsed')
    expect(wrapper.get('#tab-outline').attributes('aria-selected')).toBe('true')
  ...
  it('右栏默认折叠，点击展开与收起并保留当前题目和预览试填', async () => {
    ...
    expect(wrapper.get('.editor-grid').classes()).not.toContain('right-collapsed')
    expect(wrapper.get('button[aria-label="收起右侧面板"]').attributes('aria-expanded')).toBe('true')
    ...
    await switchRightTab('实时预览')
    expect(wrapper.get('.editor-grid').classes()).toContain('right-collapsed')
    expect(wrapper.get('.editor-grid').classes()).not.toContain('preview-expanded')
    await switchRightTab('实时预览')
    expect(wrapper.get('.editor-grid').classes()).toContain('preview-expanded')
    expect(wrapper.get('#pane-preview .el-radio input').element.checked).toBe(true)
    await wrapper.get('button[aria-label="收起右侧面板"]').trigger('click')
    expect(wrapper.get('.editor-grid').classes()).toContain('right-collapsed')
    await switchRightTab('实时预览')
    expect(wrapper.get('#pane-preview .el-radio input').element.checked).toBe(true)
  ...
  it('页面名称始终按当前位置编号，移动删除新增及保存重载后保持连续', async () => {
    expect(store.draft.pages.map(page => page.pageTitle)).toEqual(['第1页', '第2页'])
    expect(wrapper.findAll('button').some(item => item.attributes('aria-label')?.startsWith('重命名页面'))).toBe(false)
    store.movePage('P2', -1)
    expect(store.draft.pages.map(page => [page.pageCode, page.pageTitle])).toEqual([['P2', '第1页'], ['P1', '第2页']])
    store.removePage('P2')
    expect(store.draft.pages[0].pageTitle).toBe('第1页')
    store.addPage()
    expect(store.draft.pages.map(page => page.pageTitle)).toEqual(['第1页', '第2页'])
    await button('保存草稿').trigger('click')
    await flushPromises()
    expect(api.saveQuestionnaireDraft).toHaveBeenCalled()
    expect(persisted.pages.map(page => page.pageTitle)).toEqual(['第1页', '第2页'])
    await store.load(7)
    expect(store.draft.pages.map(page => page.pageTitle)).toEqual(['第1页', '第2页'])
  })
