...
test('真实HR发布、员工答题、HR完成关闭未交任务并保持已提交答案不可变', async ({ page }) => {
  ...
  const reportDrawer = page.getByRole('dialog', { name: '个人报告', exact: true })
  ...
  await page.getByRole('button', { name: '原始答案', exact: true }).click()
  await page.getByRole('button', { name: '查看答案', exact: true }).first().click()
  const answerDrawer = page.getByRole('dialog', { name: '已提交原始答案', exact: true })
  ...
  await expect(page.getByRole('heading', { name: '团队排名与明细' })).toBeVisible()
  await expect(page.getByRole('button', { name: '原始答案', exact: true })).toHaveCount(0)
  ...
  await expect(page.getByRole('button', { name: '个人报告', exact: true })).toHaveCount(1)
