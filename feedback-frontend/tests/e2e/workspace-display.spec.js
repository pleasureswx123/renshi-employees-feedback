...
test('主题切换保留输入、刷新及跨工作台恢复，弹窗和下拉菜单适配暗色', async ({ page, context }) => {
  ...
  await expectDarkSurface(page.locator('.project-filter-card'))
  ...
  await page.getByRole('button', { name: 'HR 工作台', exact: true }).click()
  await expect(page).toHaveURL(/\/hr\/projects$/)
  await expect(page.locator('.project-filter-card')).toHaveCSS('background-color', 'rgb(255, 255, 255)')
